...
export default function TaskModal({ task, onClose }) {
  ...
  const [form, setForm] = useState({
    title: '', description: '', status: 'new', // priority олиб ташланди
  ...
  useEffect(() => {
    if (task) {
      setForm({
        ...
        recurring_value: task.recurring_value || 1
      });
    }
  }, [task]);
              ...
              <div className="p-5 bg-primary-50/30 dark:bg-primary-900/10 rounded-2xl border border-primary-100 dark:border-primary-900/30 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <RefreshCw size={18} className={`transition-colors ${form.is_recurring ? 'text-primary-500 animate-spin-slow' : 'text-slate-400'}`} />
                    <div>
                ...
                {form.is_recurring && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 animate-in fade-in slide-in-from-top-2">
                    <div className="space-y-1">
                      ...
                      <select className="input h-10 text-xs font-bold rounded-xl border-primary-100" value={form.recurring_type} onChange={e => set('recurring_type', e.target.value)}>
                    ...
                    {(form.recurring_type === 'weekly') && (
                      <div className="space-y-1">
                        ...
                        <select className="input h-10 text-xs font-bold rounded-xl border-primary-100" value={form.recurring_value} onChange={e => set('recurring_value', e.target.value)}>
                          <option value="1">Душанба</option>
                          <option value="2">Сешанба</option>
                          <option value="3">Чоршанба</option>
                          <option value="4">Пайшанба</option>
                          <option value="5">Жума</option>
                          <option value="6">Шанба</option>
                          <option value="7">Якшанба</option>
                        </select>
                      </div>
                    )}
                    ...
                    {['monthly', 'quarterly', 'yearly'].includes(form.recurring_type) && (
                      <div className="space-y-1">
                        <label className="text-[9px] font-black text-primary-600 uppercase">Ой куни (1-31)</label>
                        <div className="relative">
                          <CalendarDays className="absolute left-3 top-1/2 -translate-y-1/2 text-primary-400" size={14} />
                          <input type="number" min="1" max="31" className="input h-10 pl-9 text-xs font-bold rounded-xl border-primary-100" value={form.recurring_value} onChange={e => set('recurring_value', e.target.value)} />
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-slate-50/50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-3">
                <div className="space-y-1">
                  ...
                  <select className="input h-9 text-[11px] font-bold rounded-lg border-slate-200" value={form.status} onChange={e => set('status', e.target.value)}>
                ...
                {/* Муҳимлик (priority) блоки олиб ташланди */}
