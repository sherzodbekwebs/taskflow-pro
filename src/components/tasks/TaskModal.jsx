...
// Eye ikonkasi nabludatel uchun qo'shildi
...
export default function TaskModal({ task, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    observer: task?.observer || '', // Tekshiruvchi uchun yangi maydon
  ...
  useEffect(() => {
    if (task) {
      ...
      setForm({
        ...
        observer: task.observer || '', // Tahrirlashda bazadan kelgan observerni qo'yish
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      observer: form.observer === "" ? null : form.observer, // Observerni saqlash
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[0.7rem] w-full max-w-6xl max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-800 shadow-none overflow-hidden">
        <div className="flex items-center justify-between px-8 py-5 border-b dark:border-slate-800">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Бошқарув панели / Панель управления</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="p-5 bg-primary-50/30 dark:bg-primary-900/10 rounded-2xl border border-primary-100 space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-bold dark:text-white">Такрорланувчи вазифа / Повторяющаяся задача</p>
                  </div>
                ...
                {form.is_recurring && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-primary-600 uppercase">Давр / Период</label>
                      <select className="input h-10 text-xs font-bold rounded-xl w-full" value={form.recurring_type} onChange={e => set('recurring_type', e.target.value)}>
                        <option value="daily">Ҳар куni / Ежедневно</option>
                        <option value="weekly">Ҳар ҳафталик / Еженедельно</option>
                        <option value="monthly">Ҳар ойлик / Ежемесячно</option>
                        <option value="quarterly">Ҳар чораклик / Ежеквартально</option>
                        <option value="yearly">Ҳар йиллик / Ежегодно</option>
                      </select>
                    </div>
                    ...
                    {['monthly', 'quarterly', 'yearly'].includes(form.recurring_type) && (
                      <div className="col-span-1 sm:col-span-2 grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-primary-600 uppercase">Бошланиш / Начало (1-31)</label>
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-primary-600 uppercase">Тугаш / Конец (1-31)</label>
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="bg-slate-50/50 p-5 rounded-2xl border border-slate-100 space-y-3">
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase">Ҳолат / Статус</label>
                  <select className="input h-9 text-[11px] font-bold rounded-lg w-full" value={form.status} onChange={e => set('status', e.target.value)}>
                    <option value="new">Янги / Новый</option>
                    <option value="progress">Жараёнда / В процессе</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1"><Calendar size={10} /> Қўшилган сана / Дата создания</label>
                ...
                {/* IJROCHI (ASSIGNED USER) */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase">{t.assignTo}</label>
                  <select className="input h-9 text-[11px] font-bold rounded-lg w-full" value={form.assignedUser} onChange={e => set('assignedUser', e.target.value)}>
                    <option value="">— {t.all} —</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.fullName || u.fullname}</option>)}
                  </select>
                </div>

                {/* TEKSHIRUVCHI (OBSERVER) - QO'SHILGAN QISM */}
                <div className="space-y-1">
                  <label className="text-[9px] font-bold text-slate-400 uppercase flex items-center gap-1">
                    <Eye size={10} /> Tekshiruvchi / Наблюдатель
                  </label>
                  <select className="input h-9 text-[11px] font-bold rounded-lg w-full border-blue-100 focus:border-primary-500 bg-blue-50/20" value={form.observer} onChange={e => set('observer', e.target.value)}>
                    <option value="">— {t.all} —</option>
                    {users.map(u => <option key={u.id} value={u.id}>{u.fullName || u.fullname}</option>)}
                  </select>
                </div>
