import { useState, useMemo } from 'react'; // useMemo qo'shildi
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center border border-slate-100 dark:border-slate-700">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
          Kechirasiz, ushbu amalni bajarish uchun sizda yetarli ruxsat mavjud emas. Faqat boshqaruvchilar ushbu amalni bajara olishi mumkin.
        </p>
        <button onClick={onClose} className="btn-primary w-full py-3">Tushunarli</button>
      </div>
    </div>
  );
}
...
function UserModal({ user, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    password: '', // Har doim bo'sh boshlanadi
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b dark:border-slate-700">
          <h2 className="text-lg font-bold dark:text-white">{isEdit ? "Tahrirlash" : "Yangi xodim"}</h2>
          <button onClick={onClose} className="p-2 rounded-xl text-slate-400"><X size={20} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="label">F.I.O *</label><input className="input" value={form.fullName} onChange={e => set('fullName', e.target.value)} required /></div>
          <div><label className="label">Username *</label><input className="input" value={form.username} onChange={e => set('username', e.target.value)} required /></div>
          <div>
            <label className="label">Parol {isEdit && "(o'zgartirish uchun yozing)"}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="label">Rol</label><select className="input" value={form.role} onChange={e => set('role', e.target.value)}><option value="boss">Boshliq</option><option value="worker">Ishchi</option></select></div>
            <div><label className="label">Bo'lim</label><select className="input" value={form.department} onChange={e => set('department', e.target.value)}><option value="">—</option>{departments.map((d, i) => <option key={i} value={d}>{d}</option>)}</select></div>
          </div>
          <div className="flex gap-3 pt-4"><button type="button" onClick={onClose} className="btn-secondary flex-1">Bekor qilish</button><button type="submit" className="btn-primary flex-1">Saqlash</button></div>
        </form>
      </div>
    </div>
  );
...
export default function UsersPage() {
  ...
  // TARTIBLASH MANTIQI (Admin 1-o'rin, Siz 2-o'rin)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Jami {users.length} ta foydalanuvchi</p>
        </div>
        <button 
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
        {sortedUsers.map(user => { // users.map o'rniga sortedUsers.map ishlatildi
          ...
          return (
            <div key={user.id} className="card p-6 border-transparent hover:border-primary-100 transition-all">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {isMe && <span className="text-[10px] font-bold bg-primary-100 text-primary-600 dark:bg-primary-900/30 px-2 py-0.5 rounded-full uppercase">Siz</span>}
                    </div>
              ...
              <div className="flex gap-2 mb-6">
                <span className="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-700/50 text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-300 border border-slate-100 dark:border-slate-700">
                  {user.role === 'boss' ? 'Boshqaruvchi' : 'Xodim'}
                </span>
              ...
              <div className="grid grid-cols-3 gap-2 pt-5 border-t border-slate-50 dark:border-slate-700">
                <div className="text-center"><p className="text-xl font-black dark:text-white">{stats.total}</p><p className="text-[10px] text-slate-400 font-bold uppercase">Jami</p></div>
                <div className="text-center"><p className="text-xl font-black text-green-500">{stats.completed}</p><p className="text-[10px] text-slate-400 font-bold uppercase">Bitgan</p></div>
                <div className="text-center"><p className="text-xl font-black text-amber-500">{stats.pending}</p><p className="text-[10px] text-slate-400 font-bold uppercase">Qoldi</p></div>
              </div>
            </div>
          );
        })}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl text-center">
            ...
            <h3 className="text-lg font-bold dark:text-white mb-2">O'chirishni tasdiqlang</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6 italic">"{deleteConfirm.fullName || deleteConfirm.fullname}" tizimdan butunlay o'chiriladi.</p>
            <div className="flex gap-3"><button onClick={() => setDeleteConfirm(null)} className="btn-secondary flex-1">Bekor qilish</button><button onClick={() => { deleteUser(deleteConfirm.id); setDeleteConfirm(null); }} className="btn-danger flex-1">O'chirish</button></div>
          </div>
        </div>
      )}
    </div>
